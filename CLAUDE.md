...
| `interface/editor.py` | **`document().isModified()` não diz se há o que sincronizar.** O `QSyntaxHighlighter` guarda esse sinalizador antes de reformatar e o restaura depois, para pintar não sujar o documento — ele é um segundo dono do flag. Com o realce ligado, `isModified()` voltava False logo após digitar: o texto ficava na tela e nunca chegava à tabela de peças. A bandeira própria é `editor._sujo`. |
| `interface/editor.py` | `rehighlight()` dispara `textChanged`. Sem a guarda `_repintando`, semear o realce depois de gravar marcava a aba como suja de novo — asterisco no título de um arquivo recém-salvo. |
| `janela.py` | A margem (`folga`) é limitada a **um quarto** da fatia. Com `FOLGA` fixo em 500 e `linhas_da_janela: 100`, a fatia inteira cabia dentro da margem, `precisa_deslizar` respondia sempre que sim, e cada movimento do cursor recarregava. Como recarregar repõe o cursor no começo da linha, digitar "ação" saía "oãça" — o sintoma não parece de rolagem. |
| `conversao.py` | Codecs **incrementais**, nunca `bytes.decode` por bloco: um caractere multibyte pode ficar partido entre dois blocos de 4 MB, e o defeito só aparece em arquivo grande. E `errors="strict"` dos dois lados — `replace` gravaria `?` por cima de dados que não voltam. |
| `tests/ajudantes.py` | `preparar_qt()` isola `%APPDATA%` para o processo inteiro. Já aconteceu de uma suíte montar a janela com `linhas_da_janela: 100`, a janela gravar as preferências ao fechar, e isso ir para o config **real** do usuário — quebrando outra suíte, que lia o que a primeira escreveu. |
